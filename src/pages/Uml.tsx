...
const Uml = () => {
  ...
  return (
    <>
      ...
      <h1 className="title">Liste des differents diagrammes UML classé par catégorie</h1>
      {uml.category.map((item, index) => {
        return (
          <>
            <NavLinkCategory className={"subtitle"} to={`/UML/${item.categoryName}`}> <h3>{item.categoryName}</h3></NavLinkCategory>
